...
def load_retrieval_functions(file_name_to_parsed_doc:Dict[str,ParsedDoc]):
    retrieval_functions = srv.NON_TEMPLATE_FUNCTIONS[:]
    function_name_to_file_name = {}
    
    # FUNCTIONS
    for file_name,parsed_doc in file_name_to_parsed_doc.items():
        function_description = srv.TEMPLATE_TOPIC_HANDLER['description'].format(parsed_doc.summary)

        function_name = parsed_doc.function_name
        function_name = srv.TEMPLATE_TOPIC_HANDLER['name'].format(function_name)
        function_name_to_file_name[function_name] = file_name
        f = copy.deepcopy(srv.TEMPLATE_TOPIC_HANDLER)
        f['name'] = function_name
        f['description'] = function_description
        retrieval_functions.append(f)
    return retrieval_functions, function_name_to_file_name

def load_initial_metadata(metadata:Metadata):
    metadata.file_name_to_parsed_doc = load_all_docs()
    metadata.retrieval_functions, metadata.function_name_to_file_name = load_retrieval_functions(metadata.file_name_to_parsed_doc)
    retrieval_system_count, \
    explore_all_prompt_token_count, \
    cosmos_function_name_all_prompt_token_count, \
    cosmos_system_summary_count, \
    cosmos_ownership_all_prompt_token_count = tclo.get_system_token_counts()
    retrieval_function_name_to_count = tclo.get_retrieval_function_name_to_token_count(metadata)

    metadata.token_counts = TokenCounts(retrieval_function_name_to_count=retrieval_function_name_to_count,
                                        retrieval_system_count=retrieval_system_count,
