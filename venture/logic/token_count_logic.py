...
def get_explore_all_prompt_token_count():
    explore_functions = [swbv.ANSWER_FOUND_HANDLER, swbv.MISSING_ANSWER_HANDLER]
    explore_functions_token_count = sum(count_tokens(json.dumps(f)) for f in explore_functions)
    explore_all_prompt_token_count = explore_functions_token_count + count_tokens(swbv.SYSTEM_MESSAGE) + count_tokens(swbv.USER_REMINDER)
...
def get_cosmos_ownership_all_prompt_token_count():
    cosmos_ownership_all_prompt_token_count = count_tokens(system_ownership.SYSTEM_MESSAGE) + \
                                                count_tokens(system_ownership.USER_REMINDER) + \
...
def get_system_token_counts():
    explore_retrieval_token_count = count_tokens(srv.SYSTEM_MESSAGE) + count_tokens(srv.USER_REMINDER)
    ...
    return explore_retrieval_token_count, explore_all_prompt_token_count, cosmos_function_name_all_prompt_token_count, cosmos_system_summary_token_count, cosmos_ownership_all_prompt_token_count
...
def get_retrieval_function_name_to_token_count(metadata:Metadata):
    ...
    srv.NON_TEMPLATE_FUNCTIONS
    # TOKEN COUNTS
    retrieval_function_name_to_token_count = {f['name']:count_tokens(json.dumps(f)) for f in srv.NON_TEMPLATE_FUNCTIONS}
    template_token_count = count_tokens(json.dumps(srv.TEMPLATE_TOPIC_HANDLER))
